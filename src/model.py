...
from src.features import LAG_DAYS, ROLL_WINDOWS, EXTERNAL_LAGS
...
def _prepare_Xy(df_feat: pd.DataFrame, mask: pd.Index, feature_cols: list[str], horizon: int) -> tuple[pd.DataFrame, np.ndarray]:
    X  = df_feat.loc[mask, feature_cols]
    y  = df_feat["close"].shift(-horizon).loc[mask].values
    ok = ~np.isnan(y)
    return X[ok], y[ok]

def train_direct_suite(df_feat: pd.DataFrame, train_idx: pd.Index, feature_cols: list[str]) -> dict:
...
def recursive_forecast_externals(df: pd.DataFrame, ar_models: dict[str, tuple], steps: int) -> pd.DataFrame:
    ar_lags   = [1, 2, 3, 5]
    ...
    for _ in range(steps):
        for name, (model, scaler) in ar_models.items():
            hist = histories[name]
            row  = pd.DataFrame([[hist[-lag] for lag in ar_lags]], columns=[f"lag_{l}" for l in ar_lags])
...
def walk_forward_evaluate(df_feat: pd.DataFrame, feature_cols: list[str], splits: list[tuple]) -> tuple[float, list[float]]:
    fold_mapes = []
    ...
    return avg, fold_mapes

def forecast_with_recursive_externals(df: pd.DataFrame, df_feat: pd.DataFrame, suite: dict, ar_models: dict, feature_cols: list[str]) -> pd.DataFrame:
    ...
    for h in range(FORECAST_DAYS):
        ...
        row["volatility"] = float(np.std([(means[i] - means[i-1]) / means[i-1] for i in range(1, min(10, len(means)))])) if len(means) > 1 else last_row["volatility"]
