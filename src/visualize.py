...
def plot_forecast(df: pd.DataFrame, forecast_df: pd.DataFrame, wf_mape: float):
    ...
    fig.add_trace(go.Scatter(
        x=bridge_dates, y=bridge_lower,
        mode='lines', line=dict(width=0),
        fill='tonexty', fillcolor='rgba(42, 111, 191, 0.15)',
    ...
    fig.add_trace(go.Scatter(
        x=bridge_dates, y=bridge_mean,
        mode='lines', line=dict(color=ACCENT_COLOR, width=3),
    ...
    fig.add_annotation(
        x=forecast_df["tanggal"].iloc[-1], y=pred_end,
        text=f"<b>Rp {pred_end/1_000_000:.2f}M</b><br>[{pred_lower_end/1_000_000:.2f}M – {pred_upper_end/1_000_000:.2f}M]",
        showarrow=True, arrowhead=1, arrowcolor=ACCENT_COLOR, ax=30, ay=0,
        font=dict(color=ACCENT_COLOR)
    )

    # Add marker for High / Lows
    fig.add_annotation(
        x=hist_max_date, y=hist_max, text=f"↑ High<br>Rp {hist_max/1_000_000:.2f}M",
        showarrow=True, arrowhead=2, ax=0, ay=-30, font=dict(color="#d44a4a"), arrowcolor="#d44a4a"
    )
    fig.add_annotation(
        x=hist_min_date, y=hist_min, text=f"↓ Low<br>Rp {hist_min/1_000_000:.2f}M",
        showarrow=True, arrowhead=2, ax=0, ay=30, font=dict(color="gray"), arrowcolor="gray"
    )

    # Cutoff line representing "Today"
    fig.add_vline(x=last_date.timestamp() * 1000, line_width=1, line_dash="dash", line_color="gray")

    sub = f"GC=F (IDR/gram)  |  XGBoost + DXY / US10Y / VIX  |  Walk-Forward MAPE: {wf_mape:.1f}%  |  Generated: {datetime.date.today().strftime('%d %b %Y')}"
    
    fig.update_layout(
        title=dict(text=f"<b>Gold Price Forecast — Next 3 Months</b><br><sup>{sub}</sup>", x=0.5, xanchor='center'),
        hovermode="x unified",
